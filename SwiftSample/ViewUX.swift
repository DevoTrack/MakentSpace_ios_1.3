...
public extension UIView{
    fileprivate var glowLayerAnimation : CABasicAnimation{
        let layerAnimation = CABasicAnimation(keyPath: "shadowRadius")
        layerAnimation.fromValue = 8
    ...
    var glow :Bool{
        ...
        set{
            if newValue{
                self.layer.add(self.glowLayerAnimation, forKey: "glowingAnimation")
            }else{
    ...
    public var isCurvedCorner : Bool{
        ...
        set(newValue){
            if newValue{
                self.clipsToBounds = true
                self.layer.cornerRadius = self.frame.height * (15/100)
            }else{
                self.layer.cornerRadius = 0
            }
        }
...
extension UIView{
    ...
    func transForm(){
        self.semanticContentAttribute = Language.getCurrentLanguage().getSemantic
    }
...
extension UIViewController{
    ...
    func listen2Keyboard(withView view : UIView){
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(self.KeyboardShown), name: NSNotification.Name.UIResponder.keyboardWillShowNotification, object: nil)
        
        NotificationCenter.default.addObserver(self, selector: #selector(self.KeyboardHidden), name: NSNotification.Name.UIResponder.keyboardWillHideNotification, object: nil)
    }
    func KeyboardShown(notification: NSNotification) {
        let info = notification.userInfo!
        let keyboardFrame: CGRect = (info[UIResponder.keyboardFrameEndUserInfoKey] as! NSValue).cgRectValue
        self.view.subviews.forEach { (childView) in
            print("∂tag",childView.tag)
            if childView.tag == 23{
