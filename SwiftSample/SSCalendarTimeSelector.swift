...
open class SSCalendarTimeSelector: UIViewController, UITableViewDelegate, UITableViewDataSource, createdListDelegate,SSCalendarRowProtocol, SSClockProtocol,EditPriceDelegate {
    ...
    open var arrReservedDates : NSMutableArray = NSMutableArray()
    open var arrNightlyPrice : NSMutableArray = NSMutableArray()
    ...
    // Fonts & Colors
    open var optionCalendarFontMonth = UIFont (name: Fonts.CIRCULAR_BOOK, size: 16)
    open var optionCalendarFontDays = UIFont (name: Fonts.CIRCULAR_BOOK, size: 12)
    open var optionCalendarFontToday = UIFont (name: Fonts.CIRCULAR_BOOK, size: 16)
    open var optionCalendarFontTodayHighlight = UIFont (name: Fonts.CIRCULAR_BOOK, size: 16)
    open var optionCalendarFontPastDates = UIFont (name: Fonts.CIRCULAR_BOOK, size: 16)
    open var optionCalendarFontPastDatesHighlight = UIFont (name: Fonts.CIRCULAR_BOOK, size: 16)
    open var optionCalendarFontFutureDates = UIFont (name: Fonts.CIRCULAR_BOOK, size: 16)
    open var optionCalendarFontFutureDatesHighlight = UIFont (name: Fonts.CIRCULAR_BOOK, size: 16)
    ...
    open var optionCalendarFontCurrentYear = UIFont (name: Fonts.CIRCULAR_BOOK, size: 16)
    ...
    @IBOutlet fileprivate weak var btnSaveChanges: UIButton!
...
//    @IBOutlet fileprivate weak var viewGradient: UIView!
    ...
    ///
    @IBOutlet weak var roomListingView: UIView!
    @IBOutlet weak var nightly_Btn: UIButton!
    @IBOutlet weak var nolist_Title: UILabel!
    
    @IBOutlet weak var list_Msg: UILabel!
    ...
    open override func viewDidLoad() {
        super.viewDidLoad()
        self.addBottomView()
        self.addNoListingView()
        self.roomListingView.addTap {
            self.onListTapped()
        }
        ...
        self.nolist_Title.text = self.lang.noliss_Tit
        self.list_Msg.text = self.lang.noliss_Msg
        self.btnSaveChanges.setTitle(self.lang.sav_Chgs, for: .normal)
        self.btnSaveChanges.appHostTextColor()
        self.cancel_Btn.setTitle(self.lang.cancel_Title, for: .normal)
        weekDays = [lang.sun_Title,lang.mon_Title,lang.tue_Title,lang.wed_Title,lang.thurs_Title,lang.fri_Title,lang.sat_Title]
        self.btnAvailable.setTitle(self.lang.avail_Tit, for: .normal)
        self.btnBlocked.setTitle(self.lang.blck_Tit, for: .normal)
        btnAddListing.setTitle(self.lang.addlist_Title, for: .normal)
        self.nightly_Btn.setTitle(self.lang.night_Pric, for: .normal)
        ...
        if screenHeight == 812.0 {
            rectStartBtn.origin.y = rect.size.height-btnAddListing.frame.size.height-80
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(SSCalendarTimeSelector.didRotateOrNot), name: UIDevice.orientationDidChangeNotification, object: nil)
    ...
    func addBottomView() {
        var frame = self.viewAvailableHolder.bounds
        frame = CGRect(x: 0, y: 0, width: self.view.frame.width, height: frame.height)
        self.viewAvailableHolder.frame = frame
        self.view.addBottomView(bottomView: self.viewAvailableHolder)
        viewAvailableHolder.isHidden = false
    }
    
    func addNoListingView() {
        self.roomListingView.isHidden = true
        var frame = self.viewNoListing.bounds
        frame = CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height)
        self.viewNoListing.frame = frame
        self.view.addBottomView(bottomView: self.viewNoListing)
        viewNoListing.isHidden = false
    }
    
    @objc func callAPIForGettingRooms()
    {
        self.calendarTable.isHidden = false
        var dicts = [AnyHashable: Any]()
        dicts["token"] =  Constants().GETVALUE(keyname: APPURL.USER_ACCESS_TOKEN)
        
        MakentAPICalls().GetRequest(dicts,methodName: APPURL.METHOD_CALANDER_ROOMS_LIST as NSString, forSuccessionBlock:{(_ response: Any) -> Void in
            ...
            OperationQueue.main.addOperation {
                ...
                if gModel.status_code == "1" && gModel.arrTemp1.count>0
                {
                    ...
                    self.calendarTable.isHidden = false
                    self.arrRoomList.addObjects(from: (gModel.arrTemp1 as NSArray) as! [Any])
    ...
    //MARK: API CALL FOR GETTING BLOCKED DATES
    func callAPIForCalendarAvailbilty()
    {
        self.calendarTable.isHidden = false
        var dicts = [AnyHashable: Any]()
        dicts["token"] =  Constants().GETVALUE(keyname: APPURL.USER_ACCESS_TOKEN)
        MakentAPICalls().GetRequest(dicts,methodName: APPURL.METHOD_BLOCK_DATES as NSString, forSuccessionBlock:{(_ response: Any) -> Void in
            let gModel = response as! GeneralModel
            OperationQueue.main.addOperation {
                MakentSupport().removeProgress(viewCtrl: self)
                if gModel.status_code == "1" && gModel.arrTemp2.count>0
                {
                    self.calendarTable.isHidden = false
                    self.calendarTable.delegate = self
                    self.calendarTable.dataSource = self
                    self.calendarTable.reloadData()
                    self.calendarTable.setContentOffset(CGPoint(x: 0, y:250), animated:true)
    ...
    @IBAction func onSaveTapped()
    {
        ...
        lblSaving.text = self.lang.saving_Title
    }
        ...
        MakentAPICalls().GetRequest(dicts,methodName: APPURL.METHOD_BLOCK_DATES as NSString, forSuccessionBlock:{(_ response: Any) -> Void in
            ...
            OperationQueue.main.addOperation {
                if gModel.status_code == "1"
                {
                    ...
                    self.lblSaving.text = self.lang.svd_Title
                    self.arrNightlyPrice = gModel.arrTemp2
                    self.arrReservedDates = gModel.arrTemp1
                    self.arrBlockedDates = gModel.arrTemp3
                    self.makeNewTable(gModel.arrTemp3)
                    Timer.scheduledTimer(timeInterval: 2.0, target: self, selector: #selector(self.onRemoveLabel), userInfo: nil, repeats: false)
                    self.updateRoomInfo(reservedDates: gModel.arrTemp1,nightlyPrice:gModel.arrTemp2)
                }
                else
                {
                    self.lblSaving.isHidden = true
            ...
            OperationQueue.main.addOperation {
                self.lblSaving.text = self.lang.error_Title
                Timer.scheduledTimer(timeInterval: 2.0, target: self, selector: #selector(self.onRemoveLabel), userInfo: nil, repeats: false)
                self.isRoomPriceChanged = false
            }
        })
    ...
    func updateRoomInfo(reservedDates: NSMutableArray,nightlyPrice:NSMutableArray)
    {
        var tempModel = ListingModel()
        let listModel = arrRoomList[nSelectionIndex] as? ListingModel
        tempModel = listModel!
        tempModel.blocked_dates = arrBlockedDates.mutableCopy() as? NSArray
        tempModel.reserved_dates = reservedDates.mutableCopy() as? NSArray
        tempModel.nightly_price = nightlyPrice.mutableCopy() as? NSArray
        arrRoomList.replaceObject(at: nSelectionIndex, with: tempModel)
    }
    
    @objc func onRemoveLabel()
    {
    ...
    internal func SSCalendarRowDidSelect(_ date: Date) {
        if delegate?.SSCalendarTimeSelectorShouldSelectDate?(self, date: date) ?? true {
            ...
            case .multiple:
                ...
                if arrReservedDates.count>0
                {
                    for i in 0 ..< arrReservedDates.count
                    {
                        ...
                        if strCheckInDate == strBlocked as String {
                            appDelegate.createToastMessage(self.lang.datealready_Reser, isSuccess: false)
                            return
                        }
                    }
                }
                ...
                let listModel = arrRoomList[nSelectionIndex] as? ListingModel
                strRoomPrice = listModel?.room_price as! String
                ...
                if viewAvailableHolder.isHidden
                {
                    viewAvailableHolder.isHidden = false
                    if screenHeight == 812.0 {
                        self.makeViewAnimation(yPosition: rect.size.height-self.viewAvailableHolder.frame.size.height-80 , hidden : false)
                    }
                    else{
    ...
    func manageSaveButton(_ date: Date)
    {
        ...
        if arrBlockedDates.count == 0
        {
            ...
            btnAvailable.addTarget(self, action: #selector(self.onBlockedCountZeroTapped), for: UIControl.Event.touchUpInside)
            btnBlocked.addTarget(self, action: #selector(self.onBlockedCountZeroTapped), for: UIControl.Event.touchUpInside)
        }
        else if arrBlockedDates.count == selectedDate.count
        {
            ...
            if arrBlockedDates == selectedDate
            {
                ...
                btnAvailable.addTarget(self, action: #selector(self.onBlockOneTapped), for: UIControl.Event.touchUpInside)
                btnBlocked.addTarget(self, action: #selector(self.onBlockOneTapped), for: UIControl.Event.touchUpInside)
            }
            else
            {
                for i in 0 ..< arrBlockedDates.count
                {
                    for j in 0 ..< selectedDate.count
                    {
                        ...
                        if (arrBlockedDates[i] as! String) == (selectedDate[j] as! String)
                        {
                            ...
                            btnAvailable.backgroundColor = UIColor.clear
                            btnBlocked.backgroundColor = UIColor.clear
                            btnAvailable.addTarget(self, action: #selector(self.onBlockResetTapped), for: UIControl.Event.touchUpInside)
                            btnBlocked.addTarget(self, action: #selector(self.onBlockResetTapped), for: UIControl.Event.touchUpInside)
                            return
                        }
                        else{
                            btnSaveChanges.isHidden = true
            ...
            for i in 0 ..< selectedDate.count
            {
                for j in 0 ..< arrBlockedDates.count
                {
                    ...
                    if (selectedDate[i] as! String) == (arrBlockedDates[j] as! String)
                    {
                        ...
                        btnAvailable.backgroundColor = UIColor.clear
                        btnBlocked.backgroundColor = UIColor.clear
                        btnAvailable.addTarget(self, action: #selector(self.onBlockAvailableTapped), for: UIControl.Event.touchUpInside)
                        btnBlocked.addTarget(self, action: #selector(self.onBlockAvailableTapped), for: UIControl.Event.touchUpInside)
                        return
                    }
                    else
                    {
    ...
    */
    ...
    @objc func onBlockOneTapped(sender:UIButton)
    {
        if sender.tag == 11  // Available
        {
            if isDateBlocked
            {
    ...
    @objc func onBlockResetTapped(sender:UIButton)
    {
        if sender.tag == 11
        {
    ...
    @objc func onBlockAvailableTapped(sender:UIButton)
    {
        if sender.tag == 11
        {
            btnAvailable.isSelected = true
    ...
    @objc func onBlockedCountZeroTapped(sender:UIButton)
    {
        if sender.tag == 11
        {
            btnSaveChanges.isHidden = true
    ...
    //
    // MARK: DISPLAY ALREADY CREATED LIST
    /*
        HERE WE ARE SHOWING ALREADY CREATED LIST SPACE
     */
     ...
     func onListTapped()
    {
        if arrRoomList.count > 0
        {
            ...
            let viewCreateList = StoryBoard.host.instance.instantiateViewController(withIdentifier: "CreatedList") as! CreatedList
    ...
    internal func onCreateListTapped(index: Int)
    {
        ...
        if (listModel?.room_name as! String).count > 0
        {
            lblRoomName.text = listModel?.room_name as? String
        }
        else
        {
            lblRoomName.text = (listModel?.room_location as! String).count > 0 ? String(format:"%@ \(self.lang.in_Title) %@",(listModel?.room_type)!,(listModel?.room_location)!) : String(format:"%@",(listModel?.room_type)!)
        }
        strRoomPrice = (listModel?.room_price as? String)!
        strSelectedRoomId = listModel?.room_id as! String
        strRoomCurrencyCode = listModel?.currency_code as! String
        strRoomCurrencySymbol = listModel?.currency_symbol as! String
        if (listModel?.nightly_price?.count)! > 0 {
        arrNightlyPrice = (listModel?.nightly_price?.mutableCopy()) as! NSMutableArray
        }
        if (listModel?.reserved_dates?.count)! > 0{
        arrReservedDates = (listModel?.reserved_dates?.mutableCopy()) as! NSMutableArray
        }
        lblRoomPrice.text = String(format:"%@%@",strRoomPrice,strRoomCurrencySymbol.stringByDecodingHTMLEntities)
        lblRoomPrice.appHostTextColor()
        if (listModel?.blocked_dates?.count)! > 0 {
            self.makeNewTable((listModel?.blocked_dates?.mutableCopy()) as! NSMutableArray)
        }
    }
    
    func makeNewTable(_ blocked_dates : NSMutableArray)
    {
        optionCurrentDates = []
...
//        
//        let getLastDayWeek = self.getDayOfWeek(today: optionCurrentDateRange.end.stringFromFormat("d' 'MMM' 'yyyy"))
        
//        rangeStartLabel.text = (String(format: "%@", optionCurrentDateRange.start.stringFromFormat("d' 'MMM")) as NSString) as String
...
//            }
//            else {
                cell = UITableViewCell(style: UITableViewCell.CellStyle.default, reuseIdentifier: cellIdentifierNew)
...
//                    calRow.isDateSelected = isDateSelected
                ...
                if arrReservedDates.count>0
                {
                    ...
                    for enumerator in arrReservedDates
                    {
                        calRow.arrBlockedDates.addObjects(from: [enumerator])
                    }
                    
                    /*for enumerator in arrBlockedDates
                    {
                        calRow.arrBlockedDates.addObjects(from: [enumerator])
                    }*/
...
//            }
    ...
    {
        UIView.animate(withDuration:  0.25, delay: 0.0, options: UIView.AnimationOptions.curveEaseIn, animations: { () -> Void in
...
internal class SSCalendarRow: UIView {
    ...
    let Lang = Language.getCurrentLanguage().getLocalizedInstance()
    fileprivate var days = ["S","M","T","W","T","F","S"]
    ...
    var arrReservedDates : NSMutableArray = NSMutableArray()
    var arrNightlyPrice : NSMutableArray = NSMutableArray()
    ...
    internal override func draw(_ rect: CGRect) {
        ...
        days = [Lang.sun_Sym,Lang.mon_Sym,Lang.tues_Sym,Lang.wednes_Sym,Lang.thrus_Sym,Lang.fri_Sym,Lang.sat_Sym]
        ...
        if detail.type == .month {
            ...
            let str = NSAttributedString(string: monthName, attributes: convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.font): monthFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): monthFontColor, convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle): paragraph]))
            ...
            for (index, element) in days.enumerated() {
                let str = NSAttributedString(string: element, attributes: convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.font): dayFont, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): dayFontColor, convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle): paragraph]))
...
//                        fontColor = dateTodayFontColor
                    ...
                    if comparisonDates.contains(date) {
                        ...
                        if multipleSelectionEnabled {
                            var testStringSize = NSAttributedString(string: "00", attributes: convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.font): dateTodayFontHighlight, convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle): paragraph])).size()
                            ...
                            if dateFutureFontHighlight.lineHeight > dateTodayFontHighlight.lineHeight {
                                testStringSize = NSAttributedString(string: "00", attributes: convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.font): dateFutureFontHighlight, convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle): paragraph])).size()
                            ...
                            if datePastFontHighlight.lineHeight > dateFutureFontHighlight.lineHeight {
                                testStringSize = NSAttributedString(string: "00", attributes: convertToOptionalNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.font): datePastFontHighlight, convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle): paragraph])).size()
                                dateMaxWidth = testStringSize.width
                                dateMaxHeight = testStringSize.height
                            }
                        ...
                        str = NSMutableAttributedString(string: "\(date.day.localize)")
                        ...
                        str.addAttributes(convertToNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.font): font!, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): fontHighlightColor!, convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle): paragraph]), range: NSMakeRange(0, str.length))
                        ...
                        str.addAttributes(convertToNSAttributedStringKeyDictionary([convertFromNSAttributedStringKey(NSAttributedString.Key.font): font!, convertFromNSAttributedStringKey(NSAttributedString.Key.foregroundColor): fontColor!, convertFromNSAttributedStringKey(NSAttributedString.Key.paragraphStyle): paragraph]), range: NSMakeRange(0, str.length))
                    }
